...
def test_10_previous_notice_management_flow(page: Page, login_pharm1):
    ...
    if company_info_btn.count() > 0:
        company_info_btn.first.click()
        page.wait_for_timeout(1000)

        # 그리드 셀(업체명) 클릭
        biz_cell = page.locator("xpath=(//div[@role='gridcell' and @col-id='bizName'])[1] | (//div[contains(@class, 'ag-row')]//div[contains(@col-id, 'bizName')])[1] | (//table//tbody//tr//a)[1]")
        if biz_cell.count() > 0:
