...
# --------------------------------------------------------------------------
# 계정별 로그인 Fixture (각 테스트마다 독립된 깔끔한 브라우저 세션)
# --------------------------------------------------------------------------
...
def login_cso(page: Page, credentials):
    ...
    page.wait_for_selector("xpath=//h2[contains(., '회원 업체 관리')] | //h2[contains(., '위탁 계약 관리')]", timeout=15000)
    return page

@pytest.fixture(scope="function")
def login_cso2(page: Page, credentials):
    """CSO 2 계정으로 로그인하는 공통 fixture"""
    page.goto(BASE_URL)
    page.wait_for_selector("a:has-text('회원가입')", timeout=15000)
    page.fill("input[name='email']", credentials["id_cso2"])
    page.fill("input[name='password']", credentials["password"])
    page.click("button:has-text('로그인')")
    page.wait_for_selector("xpath=//h2[contains(., '회원 업체 관리')] | //h2[contains(., '위탁 계약 관리')]", timeout=15000)
    return page
...
def login_pharm1(page: Page, credentials):
    ...
    page.fill("input[name='password']", credentials["password"])
    page.click("button:has-text('로그인')")
    page.wait_for_selector("xpath=//h2[contains(., '회원 업체 관리')] | //h2[contains(., '계약서 관리')] | //h2[contains(., '받은 재위탁 통보서')] | //h2[contains(., '받은 계약서')]", timeout=15000)
    return page

@pytest.fixture(scope="function")
def login_pharm2(page: Page, credentials):
    """제약사 2 계정으로 로그인하는 공통 fixture"""
    page.goto(BASE_URL)
    page.wait_for_selector("a:has-text('회원가입')", timeout=15000)
    page.fill("input[name='email']", credentials["id_pharm2"])
    page.fill("input[name='password']", credentials["password"])
    page.click("button:has-text('로그인')")
    page.wait_for_selector("xpath=//h2[contains(., '회원 업체 관리')] | //h2[contains(., '계약서 관리')] | //h2[contains(., '받은 재위탁 통보서')] | //h2[contains(., '받은 계약서')]", timeout=15000)
    return page
